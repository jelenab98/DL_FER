...
def fcann2_train(X, Y_, hidden_neurons=5, param_niter=1000, param_delta=0.01, param_lambda=0.05, print_step=100):
    ...
    for epoch in range(param_niter):
        ...
        grads_w_2 = np.dot(h1.T, grads_w_2_tmp)

        grads_w_1_tmp = np.dot(grads_w_2_tmp, w_2.T)
        ...
        w_1 -= param_delta * grads_w_1 + param_lambda * np.linalg.norm(w_1)/N
        b_1 -= param_delta * grads_b_1

        w_2 -= param_delta * grads_w_2 + param_lambda * np.linalg.norm(w_2)/N
